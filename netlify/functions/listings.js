...
const SITE_ID = process.env.NETLIFY_SITE_ID || '10cb2e1f-2dbd-4fdf-af97-10185ee51738';
const ADMIN_KEY = process.env.ADMIN_KEY || '12below2026admin';
function blobReq(method, key, body) {
  return new Promise((resolve, reject) => {
    const token = process.env.NETLIFY_TOKEN;
    const data = body ? JSON.stringify(body) : null;
    const path = '/api/v1/blobs/' + SITE_ID + '/listings' + (key ? '/' + key : '?prefix=listing_');
    const opts = { hostname:'api.netlify.com', path, method, headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'} };
    if (data) opts.headers['Content-Length'] = Buffer.byteLength(data);
    const r = https.request(opts, res => { let txt=''; res.on('data',c=>txt+=c); res.on('end',()=>resolve({status:res.statusCode,body:txt})); });
    r.on('error', reject);
...
exports.handler = async function(event) {
  ...
  if (event.httpMethod==='GET' && !q.action) {
    try {
      ...
      const items = await Promise.all(keys.map(async k=>{
        try { const r=await blobReq('GET',k); return JSON.parse(r.body); } catch(e){return null;}
      }));
  ...
  if (event.httpMethod==='POST') {
    try {
      ...
      listing.status = listing.status||'new';
  ...
  if (event.httpMethod==='GET' && q.action && q.id) {
    ...
    try {
      ...
      const listing = JSON.parse(ex.body);
